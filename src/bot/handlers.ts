...
export function initBot(token: string) {
  ...
  bot.setMyCommands(commands).then(() => console.log('✅ Commands registered with Telegram'));

  bot.setMyCommands(
    [
      { command: 'broadcast', description: 'Admin only: broadcast message' },
      { command: 'start', description: 'Start the bot and register yourself' },
      { command: 'signal', description: 'Generate a trade signal' },
...
⚠️ Signals are algorithmic insights, not financial advice.`,
  ...
  bot.onText(/\/clear/, async (msg) => {
    ...
    try {
      ...
      await bot.sendMessage(chatId, '✅ Redis cache and trade queue cleared.');
    } catch (err) {
